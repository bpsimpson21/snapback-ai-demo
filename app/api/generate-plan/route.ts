...
export async function POST(req: NextRequest) {
  ...
  const userPrompt = `Plan a trip for ${input.crew_size || 2} crew members from a sports media company. Destination: ${input.destination}. Arriving: ${input.arrival}. Departing: ${input.departure}. Budget: ${input.budget} total. Purpose: ${input.purpose}. They need: ${(input.needs || []).join(", ")}. Additional notes: ${input.notes}. Provide a complete, specific, actionable travel operations plan.`;
